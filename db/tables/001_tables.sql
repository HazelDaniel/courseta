...
BEGIN
  ...
  CREATE EXTENSION IF NOT EXISTS pldbgapi SCHEMA courseta;
  ...
  CREATE TABLE IF NOT EXISTS courseta.students (
    ...
    avatar_meta JSONB NOT NULL DEFAULT json_build_object('id', gen_random_uuid()),
    avatar BYTEA,
  ...
  CREATE TABLE IF NOT EXISTS courseta.lesson_contents (
    lesson_content_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    title TEXT NOT NULL,
    href TEXT NOT NULL,
    lesson_id BIGINT NOT NULL,
  ...
  ALTER TABLE courseta.courses
  ADD COLUMN exam_id UUID,
  ADD CONSTRAINT _course_exam_id_fk
  FOREIGN KEY (exam_id) REFERENCES courseta.exams(exam_id) ON DELETE SET NULL;
  ...
  CREATE TYPE courseta.ASSESSMENT_RESULT_STATUS_TYPE AS ENUM('failed', 'passed');
  ...
  CREATE TABLE IF NOT EXISTS courseta.assessments_results (
    ...
    student_id UUID NOT NULL,
    assessment_id UUID NOT NULL,
    status courseta.ASSESSMENT_RESULT_STATUS_TYPE NOT NULL,
    UNIQUE (student_id, assessment_id, attempted_at),
    FOREIGN KEY(student_id, assessment_id, attempted_at) REFERENCES courseta.students__assessments(student_id, assessment_id, submitted_at) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED
  );
  ...
  CREATE TABLE IF NOT EXISTS courseta.students__questions (
    ...
    FOREIGN KEY (student_id, assessment_id, answered_at) REFERENCES courseta.students__assessments(student_id, assessment_id, submitted_at) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED,
    FOREIGN KEY (question_id) REFERENCES courseta.questions(question_id) ON DELETE CASCADE
  );
  ...
  CREATE TABLE IF NOT EXISTS courseta.students__answers (
    ...
    FOREIGN KEY (student_id, question_id, selected_at) REFERENCES courseta.students__questions(student_id, question_id, answered_at) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED,
    FOREIGN KEY (answer_id) REFERENCES courseta.answers(answer_id) ON DELETE CASCADE
  );

  CREATE TABLE IF NOT EXISTS courseta.function_debugger (
    func_ticket TEXT NOT NULL DEFAULT '',
