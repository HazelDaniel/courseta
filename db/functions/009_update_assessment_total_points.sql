...
BEGIN
  ...
  (assessment_id UUID, new_point INT) LANGUAGE PLPGSQL AS
  $block2$
  BEGIN
    UPDATE assessments SET total_points = (total_points + new_point),
    question_count = (question_count + 1) WHERE assessment_id = assessment_id;
  END;
  $block2$;
  ...
  (assessment_id UUID, old_point INT) LANGUAGE PLPGSQL AS
  $block2$
  BEGIN
