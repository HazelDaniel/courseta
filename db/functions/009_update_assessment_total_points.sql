...
BEGIN
  ...
  CREATE OR REPLACE FUNCTION update_points_and_question_count_on_assessment () RETURNS TRIGGER AS
  $block1$
  BEGIN
    CALL update_question_equiv_assessment_points(NEW.assessment_id, NEW.points - OLD.points);
		RETURN NEW;
  END;
