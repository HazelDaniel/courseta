...
BEGIN
  ...
  BEGIN
    ...
    res := COALESCE(((acc_points::NUMERIC / equiv_assessment.total_points::NUMERIC) * 100), 0);

    UPDATE assessments_results
    SET score = res,
    status = (CASE WHEN res < equiv_assessment.pass_score THEN 'failed'::courseta.ASSESSMENT_RESULT_STATUS_TYPE ELSE 'passed'::courseta.ASSESSMENT_RESULT_STATUS_TYPE END)
  ...
  (student_id_ UUID, assessment_id_ UUID, time_attempted TIMESTAMPTZ, status_ courseta.ASSESSMENT_RESULT_STATUS_TYPE)
  RETURNS UUID
  AS
  $block2$
  DECLARE
  ...
  BEGIN
    INSERT INTO assessments_results (student_id, assessment_id, attempted_at, score, status)
    VALUES (student_id_, assessment_id_, time_attempted, 0, status_::courseta.ASSESSMENT_RESULT_STATUS_TYPE)
