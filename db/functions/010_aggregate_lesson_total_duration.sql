...
BEGIN
  ...
  CREATE OR REPLACE PROCEDURE p_01_update_lesson_content_lesson_dur_and_count
  ...
  CREATE OR REPLACE FUNCTION p_02_update_lesson_duration_and_content_count () RETURNS TRIGGER AS
  $block1$
  BEGIN
    CASE TG_OP
      WHEN 'UPDATE' THEN
        IF NEW.duration <> OLD.duration THEN
          CALL p_01_update_lesson_content_lesson_dur_and_count(NEW.lesson_id, NEW.duration - OLD.duration, 0::SMALLINT);
        END IF;
        RETURN NEW;
      WHEN 'INSERT' THEN
          CALL p_01_update_lesson_content_lesson_dur_and_count(NEW.lesson_id, NEW.duration, 1::SMALLINT);
        RETURN NEW;
      WHEN 'DELETE' THEN
        CALL p_01_update_lesson_content_lesson_dur_and_count(OLD.lesson_id, OLD.duration, -1::SMALLINT);
        RETURN OLD;
    END CASE;
  END;
