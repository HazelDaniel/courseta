...
psql -Utoughware -hlocalhost -dcourseta_test -c "INSERT INTO admins (admin_id, email, password, is_superuser) VALUES ('$CST_SUPERUSER_ID' , '$CST_SUPERUSER_EMAIL', '$CST_SUPERUSER_PASSWORD', 't') ON CONFLICT DO NOTHING;" &&\
psql -Utoughware -hlocalhost -dcourseta_test -c "INSERT INTO creators (creator_id ,email, first_name, last_name, password)\
VALUES ('$CST_SUPERUSER_ID','$CST_SUPERUSER_EMAIL' , '$CST_SUPERUSER_FIRSTNAME', '$CST_SUPERUSER_LASTNAME', '$CST_SUPERUSER_PASSWORD') ON CONFLICT DO NOTHING;" &&\
